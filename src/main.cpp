...
{            
    ...
    {        
        ...
        */
        int f_begin = 0.1*f0; // chirp initial frequency
        int f_end = 3.0*f0; // chirp end frequency
        int chirp_duration_as_steps = 1000; // chirp duration as steps
        ...
        if (t < T)
        {
            Ez[source_k] += cosf(2*M_PI*(f_begin*t + (k/2.0)*powf(t, 2.0)) + M_PI/2);            
        }
